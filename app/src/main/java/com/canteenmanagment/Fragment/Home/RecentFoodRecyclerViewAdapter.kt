...
class RecentFoodRecyclerViewAdapter(val foodList : List<Food>, val openCartDialog : (position : Int) -> Unit) : RecyclerView.Adapter<RecentFoodRecyclerViewAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        val binding = ItemFoodLayout2Binding.inflate(layoutInflater,parent,false)
    ...
    class ViewHolder(var binding: ItemFoodLayout2Binding) : RecyclerView.ViewHolder(binding.root)
